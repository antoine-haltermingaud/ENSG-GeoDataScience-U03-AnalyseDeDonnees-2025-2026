...
mode<-function(x, breaks = nclass.Sturges(x)){
  discr = cut(x,breaks)
  discrvals = as.character(discr)
  counts = sapply(levels(discr),function(l){sum(which(discrvals==l))})
  # facon aletrnative : utiliser hist(x,plot=F), puis $counts et $mids
  indmax = which(counts==max(counts))
  return(names(counts)[indmax])
}
...
starwars %>% group_by(homeworld,species) %>% 
          mutate( nombre = n(),
                  classe_taille= ifelse(height>150, "grand", "petit")
                  ) %>%  
...
starwars %>% group_by(homeworld) %>% select(homeworld,mass) %>% na.omit() %>% summarise(mass_mean = mean(mass))

# meme chose (sauf que les planètes sans observations sont incluses comme NaN dans les résultats)
starwars %>% group_by(homeworld) %>% summarise(mass_mean = mean(mass, na.rm=T))
